...
function SuccessContent() {
  ...
  useEffect(() => {
    ...
    const shouldCelebrate = paymentMethod !== "crypto";

    // Trigger confetti (card only)
    ...
    const interval = shouldCelebrate
      ...
      <main className="max-w-4xl mx-auto px-6 py-16">
        <div className={`transition-all duration-1000 ${showContent ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="text-center mb-12">
            <div className={`inline-flex items-center justify-center w-24 h-24 rounded-full mb-6 ${paymentMethod === "crypto" ? "bg-[#F59E0B]/20" : "bg-[#10B981]/20"}`}>
            ...
            <div className="flex items-center justify-center gap-2 mb-4">
              {paymentMethod !== "crypto" && <PartyPopper className="w-6 h-6 text-[#F59E0B]" />}
              <h1 className="text-4xl md:text-5xl font-bold">
                {paymentMethod === "crypto" ? "Transaction Submitted" : "You\u0027re In!"}
              </h1>
              {paymentMethod !== "crypto" && <PartyPopper className="w-6 h-6 text-[#F59E0B] scale-x-[-1]" />}
            </div>

            <p className="text-xl text-[#888] max-w-2xl mx-auto">
              {paymentMethod === "crypto" ? (
                <>
                  We&apos;ve received your transaction details for <span className="text-white font-semibold">{selectedPackage.name}</span>.
                  Our team is now verifying your payment on-chain.
                </>
              ) : (
                <>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between py-2 border-b border-[#1a1a1a]">
                  ...
                  <span className="font-medium capitalize">
                    {paymentMethod === "crypto" ? `Cryptocurrency${crypto ? ` (${crypto.toUpperCase()})` : ""}` : "Card"}
                  </span>
                </div>
                <div className="flex justify-between py-2 border-b border-[#1a1a1a]">
                ...
                {paymentMethod === "crypto" && orderId && (
                  <div className="flex justify-between py-2 border-b border-[#1a1a1a]">
                    ...
                    <span className="font-medium font-mono text-xs break-all">{orderId}</span>
                  </div>
                )}
              </div>

              <div className={`${paymentMethod === "crypto" ? "bg-[#F59E0B]/10 border border-[#F59E0B]/30" : "bg-[#10B981]/10 border border-[#10B981]/30"} rounded-lg p-4`}>
                <div className="flex items-start gap-3">
                  {paymentMethod === "crypto" ? (
                    <Clock className="w-5 h-5 text-[#F59E0B] flex-shrink-0 mt-1" />
                  ) : (
                    <Sparkles className="w-5 h-5 text-[#10B981] flex-shrink-0 mt-1" />
                  )}
                  <div>
                    <p className={`font-semibold mb-1 ${paymentMethod === "crypto" ? "text-[#F59E0B]" : "text-[#10B981]"}`}>
                      {paymentMethod === "crypto" ? "We\u0027re Verifying Your Payment" : "Your Life Just Changed"}
                    </p>
                    <p className="text-sm text-[#888]">
                      {paymentMethod === "crypto" ? (
                        <>You've done your part. Once the transaction is confirmed, we'll email your access details.</>
                      ) : (
                        <>
            ...
            <div className="space-y-6">
              <div className="flex gap-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-[#888]">
                    {paymentMethod === "crypto"
                      ? "We\u0027ll email you as soon as your payment is verified. Check your spam folder if you don\u0027t see it within 10-30 minutes."
